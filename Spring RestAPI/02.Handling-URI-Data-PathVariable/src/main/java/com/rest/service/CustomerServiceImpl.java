...
public class CustomerServiceImpl implements CustomerService {
    ...
    public String addCustomer(CustomerDTO customerDTO) throws BankException{
        ...
        Customer customer = convertDTOToEntity.mapCustomerDTOToCustomer(customerDTO);
        PlanDTO planDTO = customerDTO.getCurrentPlan();
        Plan plan = planRepository.findById(planDTO.getPlanId()).orElse(null);
        if (plan == null) {
            plan = convertDTOToEntity.createPlanFromDTO(planDTO);
            planRepository.save(plan);
        }
        customer.setCurrentPlan(plan);

        List<FriendFamily> friendFamilyList = convertDTOToEntity.mapFriendFamilyDTOList(customerDTO.getFriendAndFamily());
    ...
    public List<CustomerDTO> getAllCustomer() throws BankException{
        ...
        for(Customer customer: customerList){
            CustomerDTO customerDTO = convertEntityToDTO.mapCustomerToCustomerDTO(customer);

            Plan plan= customer.getCurrentPlan();
            if (plan != null) { // Add null check here
                PlanDTO planDTO = convertEntityToDTO.mapPlanToPlanDTO(plan);
                customerDTO.setCurrentPlan(planDTO);
            }
            customerDTO.setFriendAndFamily(convertEntityToDTO.convertFriendFamilyListToDTO(customer.getFriendAndFamily()));
    ...
    public String updateCustomer(long phoneNo, CustomerDTO customerDTO) throws BankException {
        ...
        customer = convertDTOToEntity.mapCustomerDTOToCustomer(customerDTO);
        ...
        if (friendAndFamilyDTOList != null) {
            List<FriendFamily> friendAndFamilyList = convertDTOToEntity.mapFriendFamilyDTOList(friendAndFamilyDTOList);
